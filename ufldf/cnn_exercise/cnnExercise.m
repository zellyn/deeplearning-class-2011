...
%%======================================================================
%% STEP 1: Load and visualize features learned in the linear decoder exercise
%  In this step, we load STL10Features.mat, the features you learned in the
%  linear decoder exercise, and visualize them for verification. We also
...
[numTrainImages, trainImages, trainLabels] = loadSTL10Data('trainSmall64');
...
for i = 1:1000
    ...
    [~, ~, features] = feedForwardAutoencoder(optTheta, visibleSize, hiddenSize, patch);
...
[numTestImages, testImages, testLabels] = loadSTL10Data('testSmall64');
...
pooledFeaturesTest = zeros(hiddenSize, numTestImages, floor((imageDim - patchDim + 1) / poolDim), floor((imageDim - patchDim + 1) / poolDim) );
...
softmaxModel = softmaxTrain(numel(pooledFeaturesTrain) / numTrainImages, numClasses, softmaxLambda, softmaxX, softmaxY, options);

%%======================================================================
%% STEP 4: Test classifer
...
[pred, prob] = softmaxPredict(softmaxModel, softmaxX);
acc = (pred(:) == softmaxY(:));
acc = sum(acc) / size(acc, 1);
fprintf('Accuracy: %2.3f%%\n', acc * 100);

% You should expect to get an accuracy of around 77-78% on the test images.
